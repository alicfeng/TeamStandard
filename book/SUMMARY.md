# Summary

* [团队开发规范](README.md)


* [编码协作](编码协作/README.md)
    * [单元测试](编码协作/单元测试.md)
    * [日志规范](编码协作/日志规范.md)
    * [注释规范](编码协作/注释规范.md)
    * [接口文档](编码协作/接口文档.md)
    * [代码审阅](编码协作/代码审阅.md)
    * [代码质量](编码协作/代码质量.md)
    * [Git协作工作流](编码协作/Git协作工作流.md)
    * [Git代码提交规范](编码协作/Git代码提交规范.md)
    * [Git项目分组规划](编码协作/Git项目分组规划.md)
    * [PHP代码风格统一](编码协作/PHP代码风格统一.md)
    * [统一应用模板仓库](编码协作/统一应用模板仓库.md)


* [敏捷开发](敏捷开发/README.md)
    * [容器作为底座赋能](敏捷开发/容器作为底座赋能.md)
    * [持续集成前置约定](敏捷开发/持续集成前置约定.md)
    * [持续集成协作前置](敏捷开发/持续集成协作前置.md)
    * [持续集成任务模板](敏捷开发/持续集成任务模板.md)


* [基础架构](基础架构/README.md)
    * [架构基础原则定义](基础架构/架构基础原则定义.md)
    * [框架扩展目录规约](基础架构/框架扩展目录定义.md)
    * [系统配置参数设计](基础架构/系统配置参数设计.md)
    * [平台回调机制设计](基础架构/平台回调机制设计.md)
    * [通讯报文结构设计](基础架构/通讯报文结构设计.md)
    * [Http-Api-Json设计](基础架构/Http-Api-Json设计.md)
    * [配置信息与环境变量](基础架构/配置信息与环境变量.md)
    * [Laravel框架目录规划](基础架构/Laravel框架目录规划.md)
    

* [标准SDK](标准SDK/README.md)
    * [服务通信远程调用客户端](标准SDK/服务通信远程调用客户端.md)
    * [服务通信消息队列中间件](标准SDK/服务通信消息队列中间件.md)
    * [Apidoc转Markdown文档](标准SDK/Apidoc转Markdown文档.md)
    * [Helper辅助类底层依赖库](标准SDK/Helper辅助类底层依赖库.md)
    * [Kubernetes之PHP客户端](标准SDK/Kubernetes之PHP客户端.md)
    * [Laravel框架Response库](标准SDK/Laravel框架Response库.md)
    * [Laravel框架Migrate增强](标准SDK/Laravel框架Migrate增强.md)
    * [Laravel框架Helper辅助类](标准SDK/Laravel框架Helper辅助类.md)


* [数据库](数据库/README.md)
    * [MySQL使用规范](数据库/MySQL使用规范.md)
    * [MySQL建表规约](数据库/MySQL建表规约.md)
    * [MySQL迭代维护](数据库/MySQL迭代维护.md)


* [运维规约](运维规约/README.md)
    * [行为规范](运维规约/行为规范.md)


* [面向项目](面向项目/README.md)
    * [文档编写](面向项目/文档编写.md)
    * [语义化版本管理](面向项目/语义化版本管理.md)


* [安全攻略](安全攻略/README.md)
    * [信息安全](安全攻略/信息安全.md)
    * [PHP配置安全](安全攻略/PHP配置安全.md)


* [效率工具](效率工具/README.md)
    * [CICD和DevOps](效率工具/CICD和DevOps.md)
    * [PHPStorm优秀插件集合](效率工具/PHPStorm优秀插件集合.md)
    * [那个程序员的Linux常用软件清单](效率工具/那个程序员的Linux常用软件清单.md)
    * [导出数据库结构设计markdown文档](效率工具/导出数据库结构设计markdown文档.md)


* [团队分享](团队分享/README.md)
    * [团队执行代码审阅重要性](团队分享/团队执行代码审阅重要性.md)
    * [领略代码优雅与整洁之道](团队分享/领略代码优雅与整洁之道.md)
    * [2019.11.05-CodeReview](团队分享/2019.11.05-CodeReview.md)


* [自我修养](自我修养/README.md)
    * [关爱自己](自我修养/关爱自己.md)

